...
describe('webapp/fleet/existinguser/regional-manager/booking-flow.cy.ts', () => {
    ...
    beforeEach(() => {
        ...
        fleetPage.footerNextButton().click();
        if (!lm) { servicesPage.servicesIDontKnow().should("contain", "Diagnose my problem"); }
        servicesPage.selectService(fleet.batteryReplacement).click();
        servicesPage.serviceModalPopUpPriceSpinner();
        cy.avgWait();
        servicesPage.servicesNotesButton().click().type(fleet.batteryReplacementServiceNotes, { delay: 100 });
