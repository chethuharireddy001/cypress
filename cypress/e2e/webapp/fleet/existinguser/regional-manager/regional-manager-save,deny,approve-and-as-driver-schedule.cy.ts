...
describe('webapp/fleet/existinguser/regional-manager/regional-manager-save,deny,approve-and-as-driver-schedule.cy.ts', () => {

    beforeEach(() => {

        loginPage.loginThroughCustomCredentials(fleet.migratedFleetRegionManager, fleet.migratedFleetPassword);
    ...
    it('1. login as regional manager then Save and Deny quote', () => {
        loginPage.loginThroughCustomCredentials(fleet.migratedFleetRegionManager, fleet.migratedFleetPassword);
        servicesPage.verifyFleetDashboardPage();
    ...
    it('2. login as regional manager then Save and Approve quote', () => {
        loginPage.loginThroughCustomCredentials(fleet.migratedFleetRegionManager, fleet.migratedFleetPassword);
        servicesPage.verifyFleetDashboardPage();
        quotesPage.navigateToFleetQuotesPageAndVerify();
        quotesPage.serviceDescriptionInApproveTab().then(($serviceName) => {
    ...
    it('3. login as regional manager and Deny quote', () => {
        loginPage.loginThroughCustomCredentials(fleet.migratedFleetRegionManager, fleet.migratedFleetPassword);
        servicesPage.verifyFleetDashboardPage();
        quotesPage.navigateToFleetQuotesPageAndVerify();
        quotesPage.serviceDescriptionInApproveTab().then(($serviceName) => {
