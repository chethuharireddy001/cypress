...
describe('webapp/fleet/existinguser/regional-manager/edit-profile.cy.ts', () => {
    ...
    it("1. Reset password and Verify user able to login with old passowrd", () => {
        ...
        loginPage.loginThroughCustomCredentials(fleet.migratedFleetRegionManager, fleet.newPassword);
        servicesPage.verifyFleetDashboardPage();
        zipPage.navigateToAccountPage();
        accountsPage.changePassword().click();
    ...
    it("2 .edit userinfo and test update button", () => {
        ...
        cy.reload()
        accountsPage.lastName().clear().type(fleet.originalreginmangername);
