...
describe('webapp/fleet/existinguser/driver/edit-profile.cy.tse', () => {
    ...
    it("1. Reset password and Verify user able to login with old passowrd", () => {
        ...
        loginPage.loginThroughCustomCredentials(fleet.hubdriver, fleet.newPassword);
        servicesPage.verifyFleetDashboardPage();
        zipPage.navigateToAccountPage();
        accountsPage.changePassword().click();
    ...
    it("edit userinfo and test update button", () => {
        ...
        cy.reload()
        accountsPage.lastName().clear().type(fleet.originaldrivername);
